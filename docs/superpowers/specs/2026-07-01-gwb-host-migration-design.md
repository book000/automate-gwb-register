...
`https://gwb.tomacheese.com/` で稼働するようになった。
...
1. 新規に Webhook を登録する際のデフォルト宛先 URL を新ホスト
   (`https://gwb.tomacheese.com/`) に切り替える
...
- ログ出力は新規絵文字を追加せず、既存の削除処理と同じ `🚮` を流用する
  （例: `🚮 Remove legacy webhook (hook_id=xxx, url=...)`）。
  CLAUDE.md の「既存スタイルに準拠する」方針に従い、意味的に同じ操作(Webhook 削除)には
  同一の絵文字を用いる
...
- `GWB_BASE_URL` のデフォルトを `https://gwb.tomacheese.com/` に変更
...
- `README.md` に、新規追加される「レガシー Webhook 自動移行」の挙動についての説明を追加する
  （既知の旧ホスト URL を検出した場合、`GWB_CHECK_MODE` の値に関わらず自動削除し、
  現在の `GWB_BASE_URL` 宛に Webhook を作り直す旨を明記する）
- `CLAUDE.md` の環境変数説明は現状のままで問題なし（デフォルト値の明記はしていないため）

## エージェントレビュー記録

- Codex CLI による実装レビューを試みたが、現行の認証(ChatGPT アカウント)では
  設定済みモデル(`gpt-5.4`)および代替モデル(`gpt-5-codex` / `gpt-5.1-codex` / `gpt-5`)
  がいずれも利用不可であり実行できなかった。ユーザー承認のもと、Claude 自身がレビューを実施した
- 指摘事項と対応:
  1. レガシー Webhook 削除時のログ絵文字を新規 `🔁` ではなく既存の `🚮` に統一 → 対応済み
  2. README に新規追加される自動移行挙動の説明が不足 → 対応済み（本ドキュメント更新セクションに追記）
