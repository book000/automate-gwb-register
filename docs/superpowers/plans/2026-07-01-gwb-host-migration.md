...
> **注記（エージェント実行者向け）:** 本タスクは実際の GitHub Personal Access Token と検証用リポジトリへの書き込み権限を要するため、エージェントが自律実行できない。Subagent-driven で進める場合は、このタスクをユーザー（人間）に手渡し、Step 1〜7 の手順書として提示すること。Task 1・Task 2 のコード変更・コミットはエージェントが完了させ、本タスクのみユーザー確認待ちとする。
